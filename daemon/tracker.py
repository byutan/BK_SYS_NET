...
CHANNELS = {}
# inboxes: map peer_id -> [ {from, message, ts} ] for private messages queued for peers (web peers)
INBOXES = {}
...
def handle_http(conn, addr):
    try:
        ...
        if method == 'POST' and route == '/send-private':
            # Accept private messages from web clients and attempt delivery
            try:
                obj = json.loads(content or '{}')
            except Exception as e:
                return conn.sendall(build_response(400, {'error':'invalid json', 'detail': str(e)}))
            ...
            if not to or not from_peer or message is None:
                return conn.sendall(build_response(400, {'error':'missing fields'}))
            pid = _peer_id(to)
            try:
                print(f"[Tracker] /send-private incoming -> to={pid} from={_peer_id(from_peer)} id={obj.get('id')}")
            except Exception:
                pass
            entry = {'from': from_peer, 'message': message, 'ts': time.time()}
            # preserve optional message id for web clients to dedupe
            if isinstance(obj.get('id'), str) or isinstance(obj.get('id'), int):
                entry['id'] = obj.get('id')
            # Try to forward to the peer directly if possible
            delivered = False
            try:
                with LOCK:
                    info = PEERS.get(pid)
                if info:
                    ...
                    try:
                        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                            s.settimeout(2)
                            addrp = (info.get('ip'), int(info.get('port')))
                        ...
                        delivered = False
                # if not delivered, queue in INBOXES
                if not delivered:
                    try:
                        with LOCK:
                            INBOXES.setdefault(pid, [])
                            INBOXES[pid].append(entry)
                        print(f"[Tracker] queued message for {pid} (now {len(INBOXES.get(pid,[]))} queued)")
                    except Exception as e:
                        print(f"[Tracker] failed to queue for {pid}: {e}")
            except Exception as e:
                return conn.sendall(build_response(500, {'error': str(e)}))
            status = 'forwarded' if delivered else 'queued'
            return conn.sendall(build_response(200, {'status': status}))
        ...
        if method == 'GET' and route == '/pull-inbox':
            # Query params: ip & port
            ipq = qs.get('ip', [None])[0]
            portq = qs.get('port', [None])[0]
            if not ipq or not portq:
                return conn.sendall(build_response(400, {'error':'missing ip or port'}))
            pid = f"{ipq}:{portq}"
            with LOCK:
                items = INBOXES.pop(pid, [])
            return conn.sendall(build_response(200, {'messages': items}))
        ...
        if method == 'POST' and route == '/remove-info':
            try:
                obj = json.loads(content or '{}')
            except Exception as e:
                return conn.sendall(build_response(400, {'error':'invalid json', 'detail': str(e)}))
            ...
            with LOCK:
                ...
                for ch in list(CHANNELS.keys()):
                    CHANNELS[ch]['peers'].discard(pid)
                if pid in INBOXES: INBOXES.pop(pid, None)
            print(f"[Tracker] Removed peer {pid}")
            return conn.sendall(build_response(200, {'status':'ok'}))
